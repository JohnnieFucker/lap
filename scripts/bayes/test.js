 ...
 */
function testConPro() {
    const result = Bayes.getConProb(0.3, 0.2, 0.1);
...
function testTotalProb() {
    const result = Bayes.getTotalProb(0.3, 0.7, 0.1, 0.2);
...
function testCandy() {
    ...
    const Pcb = 0.5;
    // 摸出了一颗水果糖,概率通过全概率计算
    const Pc = Bayes.getTotalProb(Pa, Pb, Pca, Pcb);
    // 求来自一号碗的概率,概率通过条件计算
    let Pac = 0;
...
function testNegative() {
    ...
    // 检查出阴性的概率通过全概率计算
    const Pd = Bayes.getTotalProb(Pa, Pb, Pda, Pdb);
    // 检查出阴性且真正健康的概率
    let Pbd = 0;
