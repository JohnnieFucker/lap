...
function testSentence(sentence, max, direction) {
    ...
    if (direction === 'asc') {
        ...
        startIndex = sentence.length - max > 0 ? sentence.length - max : 0;
    }
    const subSentence = sentence.substr(startIndex, max);
    // log(subSentence);
    if (subSentence.length === 0) {
        return;
    }
    const sigleWord = testWords(subSentence, direction);
    let sigleWordNoSymbol = sigleWord.replace(/[\n|\r|\ |\~|\`|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\-|\_|\+|\=|\||\\|\[|\]|\{|\}|\;|\:|\"|\'|\,|\<|\.|\>|\/|\?]/g, ''); // eslint-disable-line
